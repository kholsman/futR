...
#' @weblink 
#' @param dataIN      required. data.frame of 'years', 'SSB', 'R_obs' and optionally, 'sdSSB','sdR_obs'
...
#' @param covarsIN    default is NULL; data.frame of covariates (rows) of z-score scaled environmental covariates to evaluate in Rec for each year (columns). Must match dataIN row index
...
makeDat <- function(
  ...
  sigMethod = NULL) {
    ...
    rs_dat$S_obs           <-  dataIN$S_obs[ix]  #Rec[y]~  S_obs[y-1]
    rs_dat$R_obs           <-  dataIN$R_obs[ix]  #Rec[y]~  S_obs[y-1]
    #rs_dat$Ration_scaled   <-  rationIN[ix]
    if(!is.null(covarsIN)) 
      rs_dat$rs_cov        <-  covarsIN[,ix]
    rs_dat$tau             <-  tauIN
    
    if(is.null(dataIN$sdS))  { rs_dat$sdS <- rep(0,length(ix))}else{rs_dat$sdS <- dataIN$sdS}
    if(is.null(dataIN$sdR)) { rs_dat$sdR <- rep(0,length(ix))}else{rs_dat$sdR <- dataIN$sdR}
    if(is.null(covarsIN)){
      ...
      rs_dat$ncov      <-   dim(covarsIN)[1]
      if(rs_dat$ncov==1)
        rs_dat$rs_cov  <-    t(as.matrix(rs_dat$rs_cov))
    }
    
    gamma <- -2
    if(!is.null(dataIN$gamma)) gamma <- dataIN$gamma
  ...
  # 4.1 set initial conditions:
    ...
    ln_mn_rec     <-  mean(log(dataIN$R_obs[ix]))
    ln_mn_SSB     <-  mean(log(dataIN$S_obs[ix]))
