...
where $R_i$ is the "observed" or input estimates of recruitment, $\hat{R_i}$ is the estimated recruitment from a specific recruitment model (see below), and $\sigma_R$ is the process error standard deviation.  For more information see Holsman et al. 2020 Climate and trophic controls on groundfish recruitment in Alaska.
...
where $\varepsilon_i$ represents a normally distributed independent random variable with mean 0 and variance $\sigma^2_R$ (i.e., $\varepsilon_i\sim \mathrm N(0,\sigma^2_R)$, $\mathrm{a}$ is the estimated intercept, $\mathrm{b}$ is the estimated productivity or slope,$\mathbf{X}$ is a matrix of environmental covariates, where $\beta$ and $\lambda$ are vectors of estimated parameters representing covariate effects on pre- and post-spawning success, respectively, and $\theta^{\beta}$ and $\theta^{\lambda}$ are binary vectors of length $n_k$ of covariate inclusion (i.e., ${\theta^{\beta}} \in \{0,1 \}$ and ${\theta^{\lambda}} \in \{0,1 \}$). Note that in most cases, setting $a =0$ is recommended $\hat{R_i}\sim0$ as $\hat{S_i}\to0$)
...
As a default, each model fits lognormally distributed process error for recruitment ($\sigma_R$) using maximum likelihood. However, there may be additional observation error associated with the input estimates of recruitment or spawning that result from model mispecification or sampling:

$$log R_i\sim N \left(log\bar{R}_i,\tau \right)$$

where $R_i$ is the "observed" or input recruitment, $\bar{R}_i$ is the "true" recruitment, and $\tau$ is the observation error.

Various options for observation error on recruitment or spawning estimates are available through the following "sigMethod" selections:
...
```{r fitR2, echo=T,eval=F,message=F,results='hide'}
 ...
 #FIXME: updatePlots = TRUE; W = 7; H1 = 6
