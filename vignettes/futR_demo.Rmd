...
```{r setup3, echo=T,eval=F,message=F,results='hide'}
 ...
 # read in the data and create a datlist
  futR_path <- ".."
  datlist <- futR::makefutR_data(fn = file.path(futR_path,"data-raw","in","futR_Inputs.xlsx" ), export_all=F)
...
```{r fitR1, echo=T,eval=F,message=F,results='hide'}
  ...
  Rec1 <-  mm <-futR::runRecMod(dlistIN   = datlist,
                          version   = 'futR',
                          src_fldr   = file.path(futR_path,"src","TMB"),
                          recompile = FALSE,
                          simulate  = TRUE,
                          sim_nitr  = 1000)
   # summarize results
...
# source("R/make.R")
 # copy and paste this into R window (won't work within markdown)
  tmpwd<- getwd()
  setwd(file.path(tmpwd,"vingettes"))
